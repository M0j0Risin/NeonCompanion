...
/// its <c>profile.json</c> and its memories (<see cref="Profiles.BasicSidekickFiles"/> — the user's call
/// 2026-09-15; the settings alone before); <c>advanced</c> copies the prompt files too
/// (<see cref="Profiles.SidekickFiles"/>, each when it exists). <see cref="Resolve"/> is the one place
...
public static class NewProfileMode
{
    ...
    /// <summary>
    /// The sidekick files <c>/profile add</c> copies: every one under <c>advanced</c>
    /// (<paramref name="copyPromptFiles"/> true, <see cref="Profiles.SidekickFiles"/>), the memories
    /// alone under <c>basic</c> (<see cref="Profiles.BasicSidekickFiles"/>).
    /// </summary>
    public static IReadOnlyList<string> FilesFor(bool copyPromptFiles) =>
        copyPromptFiles ? Profiles.SidekickFiles : Profiles.BasicSidekickFiles;
