...
public sealed class AppSettings : IDisposable
{
    ...
    public const string DefaultDirectoryName = ".neoncompanion";
    ...
    /// <summary>The models directory: <c>models</c> under the home, so <c>NEONCOMPANION_HOME</c> moves both; shared by every profile.</summary>
    ...
    /// <summary>
    /// Resolves the settings directory: an explicit override (<c>NEONCOMPANION_HOME</c>), else
    /// <c>%USERPROFILE%\.neoncompanion</c>, else next to the binary when no profile exists.
    /// </summary>
    ...
    /// rewritten, the companion's files untouched since 2026-09-20). For the loaded
