...
public static partial class SmokeChecks
{
    ...
    public static SmokeCheck ProbeImageResize()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), "neoncompanion-smoke-" + Guid.NewGuid().ToString("N") + ".bmp");
        try
        {
    ...
    {
        var bogusModel = Path.Combine(Path.GetTempPath(), "neoncompanion-no-such-model-" + Guid.NewGuid().ToString("N") + ".bin");
        string outcome;
        try
        {
    ...
    public static SmokeCheck ProbeWhisperVad()
    {
        ...
        var bogusModel = Path.Combine(Path.GetTempPath(), "neoncompanion-no-such-vad-" + Guid.NewGuid().ToString("N") + ".bin");
        try
        {
    ...
    public static SmokeCheck ProbeSessions()
    {
        ...
        string dir = Path.Combine(Path.GetTempPath(), "NeonCompanion.smoke", Guid.NewGuid().ToString("N"));
        try
        {
            string version;
