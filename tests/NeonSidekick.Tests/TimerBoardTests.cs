...
public class TimerBoardTests : IDisposable
{
    ...
    public void StartRingStop_EachLogOneLine()
    {
        ...
        Action<NeonSidekick.Diagnostics.DiagnosticEvent> capture = e => { if (e.Category == TimerBoard.Category) lines.Add(e); };
        ...
        Assert.Equal(NeonSidekick.Diagnostics.DiagnosticLevel.Debug, lines.Single(e => e.Message.EndsWith("acknowledged", StringComparison.Ordinal)).Level);
        Assert.All(lines.Where(e => !e.Message.EndsWith("acknowledged", StringComparison.Ordinal)), e => Assert.Equal(NeonSidekick.Diagnostics.DiagnosticLevel.Info, e.Level));
