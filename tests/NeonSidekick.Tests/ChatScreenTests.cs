...
/// interactive <see cref="SidekickAppTests"/>: a 240-column console, every key pushed up front,
...
public partial class ChatScreenTests : IDisposable
{
    ...
    public void ProfileStrings_ArePinned()
    {
        ...
        Assert.Equal("(🪪 created profile \"x\" from the current settings, memories, persona, operating rules, voice directive, MCP servers)", ChatScreen.ProfileCreatedNotice("x", Profiles.SidekickFiles));
    ...
    private void FurnishedDefaultProfile()
    {
        ...
        File.WriteAllText(Path.Combine(_settings.ProfileDirectory, OperataFile.FileName), "Answer in haiku.");
        File.WriteAllText(Path.Combine(_settings.ProfileDirectory, VocaliaFile.FileName), "Speak like a pirate.");
        File.WriteAllText(Path.Combine(_settings.ProfileDirectory, McpConfigFile.FileName), McpConfigFile.EmptyText);   // the fifth sidekick file (2026-09-20)
    }

    [Fact]
    public async Task Profile_Add_Advanced_CopiesTheSidekickFiles_AndTheNewProfileUsesThem()
    {
        ...
        Assert.Contains("  · " + ChatScreen.ProfileCreatedNotice("work", Profiles.SidekickFiles), output);
        Assert.Contains("  · " + SettingsMenu.SwitchedNotice("work"), output);
        Assert.Equal("work", _settings.ProfileName);
        Assert.Equal(
    ...
    public async Task Profile_Add_Basic_CopiesTheSettingsAndTheMemories()
    {
        ...
        Assert.Contains("  · " + ChatScreen.ProfileCreatedNotice("work", Profiles.BasicSidekickFiles), output);
    ...
    public async Task Profile_Add_BadOrExistingName_IsRefused()
    {
        ...
        PushLine("/profile add Neon");   // the sidekick's own name (2026-09-21)
        PushLine("/profile add WORK");
        PushLine("/exit");
    ...
    public async Task WithGeometry_AboutOpensTheInfoPane_ThreeTabs_AndEscClosesIt()
    {
        ...
        // Nothing in the transcript; ESC brings the input row back.
        Assert.DoesNotContain("  · NeonSidekick", output);
        Assert.EndsWith(rule + "\n" + InputLine.PromptGlyph + ChatScreen.InputPlaceholder + "\n" + rule + "\n" + Row(ChatScreen.HintLine(null)) + "\n", raw);
    }
    ...
    public async Task WithoutGeometry_AboutPrintsTheThreeTabs()
    {
        ...
        Assert.Contains("  ·   NeonSidekick " + SidekickApp.Version, output);
    ...
    {
        _overriddenBy = f => f == SettingsField.WorkingDirectory ? SidekickOptions.CwdFlag : null;
        string elsewhere = Path.Combine(_dir, "elsewhere");
        PushLine("/cwd");
        PushLine("/cwd " + elsewhere);
        PushLine("/exit");
        ...
        Assert.Contains("  · Working directory (cwd): " + elsewhere, output);
        Assert.Contains(SettingsMenu.OverrideNotice(SidekickOptions.CwdFlag), output);
