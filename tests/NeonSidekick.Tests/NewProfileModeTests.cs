...
public class NewProfileModeTests
{
    ...
    [Fact]
    public void FilesFor_IsEverySidekickFileAdvanced_AndTheMemoriesAloneBasic()
    {
        Assert.Same(Profiles.SidekickFiles, NewProfileMode.FilesFor(true));
        Assert.Same(Profiles.BasicSidekickFiles, NewProfileMode.FilesFor(false));
        Assert.Equal(new[] { "memory.json" }, Profiles.BasicSidekickFiles);
        Assert.All(Profiles.BasicSidekickFiles, f => Assert.Contains(f, Profiles.SidekickFiles));
    }
