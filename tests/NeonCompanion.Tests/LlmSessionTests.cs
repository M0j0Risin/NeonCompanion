...
public class LlmSessionTests
{
    ...
    public async Task Connect_NoServer_ReturnsFalse_WithNoEndpoint()
    {
        ...
        Assert.Equal("LLM: no server found on 127.0.0.1 ports 1234, 8000, 30000, 8080, 11434, 8888; set NEONCOMPANION_LLM_URL.", LlmSession.NoServerLine(ScanScope.Local));
        Assert.Equal("LLM: no server found on the local network (ports 1234, 8000, 30000, 8080, 11434, 8888); set NEONCOMPANION_LLM_URL.", LlmSession.NoServerLine(ScanScope.Remote));
        Assert.Equal("LLM: no server found on 127.0.0.1 or the local network (ports 1234, 8000, 30000, 8080, 11434, 8888); set NEONCOMPANION_LLM_URL.", LlmSession.NoServerLine(ScanScope.Both));
        Assert.Equal("LLM: no URL is set and LLM scan mode is disabled; set NEONCOMPANION_LLM_URL, or the URL or the scan mode in /settings.", LlmSession.NoServerLine(ScanScope.Disabled));
    }
    ...
    public async Task Connect_FactoryThrows_IsAnError_EndpointStaysKnown()
    {
        ...
        void Capture(NeonCompanion.Diagnostics.DiagnosticEvent e) => seen.Add($"[{e.Category}] {e.Message}");
        NeonCompanion.Diagnostics.DiagnosticLog.Emitted += Capture;
        bool echo = NeonCompanion.Diagnostics.DiagnosticLog.EchoToConsole;
        NeonCompanion.Diagnostics.DiagnosticLog.EchoToConsole = false;
        try
        {
            Assert.False(await session.ConnectAsync(new AppSettingsData(), CancellationToken.None));
        }
        ...
        {
            NeonCompanion.Diagnostics.DiagnosticLog.Emitted -= Capture;
            NeonCompanion.Diagnostics.DiagnosticLog.EchoToConsole = echo;
        }
