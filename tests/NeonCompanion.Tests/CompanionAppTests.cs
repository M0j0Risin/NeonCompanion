...
public class CompanionAppTests : IDisposable
{
    ...
    private async Task<string> Headless(string input, EnvironmentOverrides? env = null, Func<LlmEndpoint, LlmTimeouts, IChatClient>? chat = null)
    {
        ...
        int code = await app.RunAsync(CompanionOptions.None with { Headless = true }, CancellationToken.None);
        Assert.Equal(0, code);
    ...
    public void RenderBanner_ShowsTitleVersionAndRule_NoKeyHints()
    {
        ...
        Assert.Contains("N E O N   C O M P A N I O N", output);
        Assert.Contains("N E O N   C O M P A N I O N  v" + CompanionApp.Version, output);   // the version shares the title line
    ...
    public async Task Smoke_AllPass_ReturnsZero()
    {
        ...
        int code = await app.RunAsync(CompanionOptions.None with { Smoke = true }, CancellationToken.None);
    ...
    public async Task AudioCheck_UsesThePlaybackFactory_AndExitsWithItsCode()
    {
        ...
        int code = await app.RunAsync(CompanionOptions.None with { AudioCheck = true }, CancellationToken.None);
        ...
        Assert.Contains("N E O N   C O M P A N I O N", _console.Output);
    }
    ...
    public async Task VoiceCheck_Heard_ExitsZero_ThroughTheFactories()
    {
        ...
        int code = await app.RunAsync(CompanionOptions.None with { VoiceCheck = true }, CancellationToken.None);
    ...
    public async Task Headless_LogsItsTurnsToTheSessionStore_AndNewStartsAnother()
    {
        ...
        using var store = new NeonCompanion.Sessions.SessionStore(_settings.ProfileDirectory);
        ...
        Assert.Equal(2, NeonCompanion.Sessions.SessionHistory.FromJson(record.HistoryJson).Count(ConversationHistory.IsTurnStart));
    }
    ...
    public async Task Headless_SessionLoggingOff_WritesNothing()
    {
        ...
        Assert.False(File.Exists(Path.Combine(_settings.ProfileDirectory, NeonCompanion.Sessions.SessionStore.FileName)));
    }
    ...
    public async Task Headless_StreamsARealReplyPerLine_AndExitsOnEof()
    {
        ...
        Assert.Contains(CompanionApp.VersionLine, output);
        Assert.Contains(CompanionApp.HeadlessHint, output);
    ...
    public async Task Headless_NoServer_SaysSo_AndAnswersEveryLineWithAnError()
    {
        ...
        Assert.Contains(CompanionApp.HeadlessNoServerLine(ScanScope.Local), output);
        Assert.Contains("Neon: " + CompanionApp.HeadlessNoAssistantReply, output);
    ...
    public async Task Headless_ScanDisabled_SaysSo_AndAsksNothing()
    {
        ...
        Assert.Contains(CompanionApp.HeadlessNoServerLine(ScanScope.Disabled), output);
        Assert.Contains("Neon: " + CompanionApp.HeadlessNoAssistantReply, output);
    ...
    {
        Assert.Equal("LLM: no server found on 127.0.0.1 ports 1234, 8000, 30000, 8080, 11434, 8888; set NEONCOMPANION_LLM_URL.", CompanionApp.HeadlessNoServerLine(ScanScope.Local));
        Assert.Equal("[error] No LLM endpoint. Set NEONCOMPANION_LLM_URL and restart.", CompanionApp.HeadlessNoAssistantReply);
    ...
    public async Task Headless_Compact_SummarisesTheOlderTurns_AndNoticesOnTheReplyLine()
    {
        ...
        Assert.Equal("Headless mode. Type a message; /clear or /new forgets the conversation; /compact [focus] shrinks it; /exit or EOF exits.", CompanionApp.HeadlessHint);
    ...
    /// </summary>
    private async Task<string> InteractiveAsync(EnvironmentOverrides? env = null, CompanionOptions? options = null, Func<LlmEndpoint, LlmTimeouts, IChatClient>? chat = null, CancellationToken cancellationToken = default)
    {
        _console.Interactive();
        _console.Profile.Width = 240;
        int code = await App(env, chat: chat).RunAsync(options ?? CompanionOptions.None, cancellationToken);
        Assert.Equal(0, code);
    ...
    public async Task Interactive_Clear_ForgetsTheConversation()
    {
        ...
        Assert.Equal(2, output.Split("N E O N   C O M P A N I O N").Length - 1);
        Assert.Equal(2, output.Split("LLM: ").Length - 1);
        Assert.DoesNotContain("(conversation cleared)", output);
        Assert.True(output.LastIndexOf("N E O N   C O M P A N I O N", StringComparison.Ordinal) < output.LastIndexOf("› b", StringComparison.Ordinal));
    }
    ...
    public async Task Interactive_StartsOnAClearedScreen_SmokeDoesNot()
    {
        ...
        Assert.True(erase < output.IndexOf("N E O N   C O M P A N I O N", StringComparison.Ordinal));

        using var smokeConsole = new TestConsole();
        var app = new CompanionApp(smokeConsole, _settings, EnvironmentOverrides.Empty, smokeChecks: () => new[] { new SmokeCheck("a", true, "ok") });
        Assert.Equal(0, await app.RunAsync(CompanionOptions.None with { Smoke = true }, CancellationToken.None));
        Assert.DoesNotContain(EraseDisplay, smokeConsole.Output);
        Assert.Contains("N E O N   C O M P A N I O N", smokeConsole.Output);
    }
    ...
    public async Task Interactive_SpeechServerFound_ProbesOnce_AndPrintsNothingUnderThePanel()
    {
        ...
        int code = await App(synth: _ => synth).RunAsync(CompanionOptions.None, CancellationToken.None);
    ...
    public async Task Interactive_UrlFlag_BeatsTheVariable()
    {
        ...
        string output = await InteractiveAsync(env, CompanionOptions.None with { Url = "http://127.0.0.1:1234" });
    ...
    public async Task Interactive_CancelledToken_ExitsZero_InBothModes()
    {
        ...
        Assert.Equal(0, await App().RunAsync(CompanionOptions.None with { Headless = true }, cts.Token));
        _console.Interactive();
        Assert.Equal(0, await App().RunAsync(CompanionOptions.None, cts.Token));
    }
    ...
    {
        Assert.StartsWith(CompanionApp.Name + " ", CompanionApp.VersionLine);
        Assert.Equal(3, CompanionApp.Version.Split('.').Length);
    }
    ...
    public void RenderBanner_IsTitleAndOneRule_NoStatusRows()
    {
        ...
        int title = Array.FindIndex(lines, l => l.Contains("N E O N   C O M P A N I O N", StringComparison.Ordinal));
    ...
    public void RenderBanner_ShowsTheWorkingDirectoryAtTheRightEdge_ByDefault()
    {
        ...
        Assert.StartsWith("  N E O N   C O M P A N I O N  v" + CompanionApp.Version + "  ", title);   // the version still shares the line, then the gap
        Assert.EndsWith(CompanionApp.BannerPath(expected, 100 - TextCells.Width("  N E O N   C O M P A N I O N  v" + CompanionApp.Version) - CompanionApp.BannerPathGap), title);
    ...
    public async Task RenderBanner_ShowsTheConfiguredDirectory_AndTheCwdFlagOverIt()
    {
        ...
        await App(smoke: () => []).RunAsync(CompanionOptions.None with { Smoke = true, WorkingDirectory = Path.Combine(_dir, "flagged") }, CancellationToken.None);
    ...
    public void RenderBanner_ShowWorkingDirectoryOff_IsTheTitleAndTheVersionAlone()
    {
        ...
        string title = TitleLine();
        Assert.Equal("  N E O N   C O M P A N I O N  v" + CompanionApp.Version, title.TrimEnd());
    ...
    {
        string left = "  N E O N   C O M P A N I O N  v1.2.3";   // 37 cells

        // No path: today's line, nothing appended.
        Assert.Equal(Theme.GradientMarkup("  N E O N   C O M P A N I O N") + Theme.DimMarkup("  v1.2.3"), CompanionApp.BannerTitleMarkup("1.2.3", null, 80));
        Assert.Equal(CompanionApp.BannerTitleMarkup("1.2.3", null, 80), CompanionApp.BannerTitleMarkup("1.2.3", "  ", 80));

        // A path that fits: blanks up to the right edge, the path dim; the whole line exactly the width.
        string markup = CompanionApp.BannerTitleMarkup("1.2.3", @"D:\Repo\NeonCompanion", 80);
        string plain = Markup.Remove(markup);
        Assert.Equal(80, TextCells.Width(plain));
        Assert.StartsWith(left + "  ", plain);
        Assert.EndsWith(@"D:\Repo\NeonCompanion", plain);
        Assert.EndsWith(Theme.DimMarkup(@"D:\Repo\NeonCompanion"), markup);

        // A long path is cut from the front with the gap kept; a bracket in the path is escaped, not markup.
        string cut = Markup.Remove(CompanionApp.BannerTitleMarkup("1.2.3", @"D:\Some\Very\Long\Folder\Tree\That\Never\Ends\NeonCompanion", 80));
        Assert.Equal(80, TextCells.Width(cut));
        Assert.StartsWith(left + "  …", cut);
        Assert.EndsWith(@"\NeonCompanion", cut);
        Assert.EndsWith("[x]", Markup.Remove(CompanionApp.BannerTitleMarkup("1.2.3", @"D:\a\[x]", 80)));

        // Under BannerPathMinCells of room the path is left off: the title and the version alone again.
        Assert.Equal(CompanionApp.BannerTitleMarkup("1.2.3", null, 46), CompanionApp.BannerTitleMarkup("1.2.3", @"D:\Repo", 37 + CompanionApp.BannerPathGap + CompanionApp.BannerPathMinCells - 1));
        Assert.EndsWith("  …ion\\src", Markup.Remove(CompanionApp.BannerTitleMarkup("1.2.3", @"D:\Repo\NeonCompanion\src", 37 + CompanionApp.BannerPathGap + CompanionApp.BannerPathMinCells)));   // exactly the minimum room
    }
    ...
    [InlineData(@"D:\Repo\NeonCompanion", 40, "D:\\Repo\\NeonCompanion")]
    [InlineData(@"D:\Repo\NeonCompanion", 10, "…Companion")]
    public void BannerPath_IsPinned(string? path, int room, string expected) => Assert.Equal(expected, CompanionApp.BannerPath(path, room));
    ...
    public async Task Interactive_CwdFlag_OverridesTheSetting()
    {
        ...
        string output = await InteractiveAsync(options: CompanionOptions.None with { WorkingDirectory = launch });
        ...
        Assert.Contains(CompanionOptions.CwdFlag, output);   // the /cwd notice's tag
        Assert.Contains(ChatScreen.CwdNotice(launch, false, CompanionOptions.CwdFlag), output);
    ...
    {
        var facts = new AboutFacts("0.2.0", ".NET 10.0.0", true, "x64", "Microsoft Windows 10.0.26200", null, @"C:\h\.neoncompanion", @"C:\h\.neoncompanion\profiles\default", @"C:\h\.neoncompanion\models");
        Assert.Equal(@"NeonCompanion 0.2.0 (native, .NET 10.0.0, x64, Microsoft Windows 10.0.26200), mode interactive, flags --cwd D:\x, home C:\h\.neoncompanion, profile default, console 240×60",
            CompanionApp.StartupLogLine(facts, CompanionOptions.Parse(["--cwd", @"D:\x"]), "default", 240, 60));
        Assert.Equal(@"NeonCompanion 0.2.0 (JIT, .NET 10.0.0, x64, Microsoft Windows 10.0.26200), mode headless, no flags, home C:\h\.neoncompanion, profile work, console 80×25",
            CompanionApp.StartupLogLine(facts with { NativeAot = false }, CompanionOptions.Parse(["--headless"]), "work", 80, 25));
        Assert.Equal("Startup", CompanionApp.StartupCategory);
        Assert.Equal("Overrides in force: NEONCOMPANION_TTS_SPEED=1.3", CompanionApp.EnvironmentLogLine("NEONCOMPANION_TTS_SPEED=1.3"));
        Assert.Equal("Headless exit: end of input", CompanionApp.HeadlessExitLogLine(ChatScreen.ExitByEndOfInput));
    }
    ...
    public async Task Headless_LogsTheStartup_TheTurn_AndTheExit()
    {
        ...
        Action<NeonCompanion.Diagnostics.DiagnosticEvent> capture = e => { if (e.Category is CompanionApp.StartupCategory or Assistant.TurnCategory or ChatScreen.AppCategory) lines.Add(e); };
        ...
        var startup = Assert.Single(lines, e => e.Category == CompanionApp.StartupCategory);
        Assert.Equal(NeonCompanion.Diagnostics.DiagnosticLevel.Info, startup.Level);
        Assert.StartsWith("NeonCompanion " + CompanionApp.Version + " (", startup.Message, StringComparison.Ordinal);
        ...
        Assert.Contains(lines, e => e.Category == ChatScreen.AppCategory && e.Level == NeonCompanion.Diagnostics.DiagnosticLevel.Info && e.Message == CompanionApp.HeadlessExitLogLine(ChatScreen.ExitByEndOfInput));
    }
}
