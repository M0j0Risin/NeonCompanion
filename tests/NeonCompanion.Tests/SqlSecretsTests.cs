...
/// Credential Manager entry a test writes is under a fresh <c>NeonCompanion.Tests/…</c> target and removed after.
/// </summary>
public sealed class SqlSecretsTests : IDisposable
{
    ...
    private readonly string _target = "NeonCompanion.Tests/" + Guid.NewGuid().ToString("N");
    ...
    public void AnAccount_SplitsAsLogonUserTakesIt_AndANetOnlyLogon_KeepsTheLocalIdentity()
    {
        ...
        using var token = WindowsCredentials.LogonNetOnly(@"NEONCOMPANION-TEST\nobody", "x", out string? error);
        Assert.Null(error);
    ...
    public void Resolve_ReadsTheConnectionsStore_AndNamesTheFixWhenItIsEmpty()
    {
        ...
        Assert.Equal("NeonCompanion/sql/prod", new SqlConnectionConfig().CredentialTarget("prod"));
    }
