...
public sealed class WorkingDirectoryTests : IDisposable
{
    ...
    public void ReadImage_Ok_Missing_Directory_NotAnImage_TooBig_Outside_Trash()
    {
        ...
        File.WriteAllBytes(Full(@"shots\square.bmp"), NeonCompanion.App.SmokeChecks.SolidBmp(4, 4));
        Put("notes.txt", "not a picture");
        Directory.CreateDirectory(Full("dir"));
        Directory.CreateDirectory(Full(WorkingDirectory.TrashFolderName));
        File.WriteAllBytes(Full(WorkingDirectory.TrashFolderName + @"\old.bmp"), NeonCompanion.App.SmokeChecks.SolidBmp(2, 2));
    ...
    public void TrashMoves_LogTheirCopies_AndTheLinesArePinned()
    {
        ...
        Action<NeonCompanion.Diagnostics.DiagnosticEvent> capture = e => { if (e.Category == WorkingDirectory.Category && (e.Message.StartsWith("Trashed ", StringComparison.Ordinal) || e.Message.StartsWith("Restored ", StringComparison.Ordinal) || e.Message.StartsWith("Trash emptied", StringComparison.Ordinal))) lines.Add(e); };
        ...
        Assert.Equal((NeonCompanion.Diagnostics.DiagnosticLevel.Info, "Trash emptied: 1 files, 1 folders, 1 bytes"), (lines[3].Level, lines[3].Message));
        Assert.All(lines.Take(3), e => Assert.Equal(NeonCompanion.Diagnostics.DiagnosticLevel.Debug, e.Level));
