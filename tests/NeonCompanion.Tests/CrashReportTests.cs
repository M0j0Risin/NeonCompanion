...
public class CrashReportTests : IDisposable
{
    ...
    public void Notice_AndLogLine_ArePinned()
    {
        ...
        Assert.Equal($"NeonCompanion crashed: ArgumentOutOfRangeException: {ex.Message}. Details in C:\\x\\crash.log.", CrashReport.Notice("C:\\x\\crash.log", ex));
        Assert.Equal($"NeonCompanion crashed: ArgumentOutOfRangeException: {ex.Message}. Details could not be written.", CrashReport.Notice(null, ex));
